...
function openPdf(data) {
  var requestOptions = {
    method: 'PUT',
    redirect: 'follow'// Ändern Sie die Methode auf 'GET', da Sie die PDF nur abrufen möchten.
  };

  fetch(data, requestOptions)
    .then(response => response.blob()) // Erhalten Sie die PDF als Blob-Objekt.
    .then(pdfBlob => {
      const url = URL.createObjectURL(pdfBlob); // Erstellen Sie eine URL für das Blob-Objekt.
      window.open(url, '_blank'); // Öffnen Sie die PDF in einem neuen Browserfenster.
    })
    .catch(error => console.log('Fehler beim Öffnen der PDF', error));
}
